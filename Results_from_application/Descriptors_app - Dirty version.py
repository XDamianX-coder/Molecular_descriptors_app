...
#def get_desc(Mformula):

    #data = pcp.get_properties(['isomeric_smiles','xlogp','rotable_bond_count'],Mformula,'formula',as_dataframe=True)
    #return data
#def mordred_descriptors(smiles):
    #baseData_4 = []
    #for i in smiles:
        #baseData_4.append(Chem.MolFromSmiles(smiles))
    #calc = Calculator(descriptors, ignore_3D=True)
    #df = calc.pandas(baseData_4)
    #df = np.arange(1, 1)
    ##i = 0
    #row = np.array([descriptors])
    #if (i == 0):
       # df = row
   # else:
      #  df = np.vstack([df, row])
   # i = i + 1
   # columnNames = [descriptors]
   # mordred = pd.DataFrame(data=df, columns=columnNames)
   # return mordred
def mordred_descriptors(smiles):
    molObjs = []
    for i in smiles:
        molObjs.append(Chem.MolFromSmiles(smiles))
    calc = Calculator(descriptors, ignore_3D=True)
    df = calc.pandas(molObjs)
    return df


#time.sleep(2)
#image = Image.open('compound1.png')
#time.sleep(1)
#image1 = Image.open('compound2.png')
#time.sleep(1)
#image2 = Image.open('compound3.png')

#def OpenImage(image):
    #openimages =[]
    #for i in filenames:
        #img = Image.open(i)
        #openimages.append(img)
...
#Mformula = "C10H22"
#Mformula_1 = st.sidebar.text_area("SMILES input to get CID", Mformula)
#Mformula_1 = Mformula_1.split('\n')
#st.header('Input formula to get the compound properties.')
#Mformula
#A1 = get_desc(Mformula_1)
#A1
